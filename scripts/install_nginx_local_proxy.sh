...
if [ "$EUID" -ne 0 ]; then
  echo "ERROR: Please run as root or with sudo (e.g., sudo bash scripts/install_nginx_local_proxy.sh)"
  exit 1
fi
...
echo "Installing local Nginx sandbox proxy..."
cp "$PROJECT_DIR/deploy/nginx-ghbs-local-sandbox.conf.example" /etc/nginx/sites-available/ghbs-trading-local-sandbox.conf

ln -sfn /etc/nginx/sites-available/ghbs-trading-local-sandbox.conf /etc/nginx/sites-enabled/ghbs-trading-local-sandbox.conf

echo "Validating Nginx configuration..."
if nginx -t; then
    echo "Nginx configuration is valid. Reloading Nginx..."
    systemctl reload nginx
    echo "Local Nginx sandbox proxy successfully installed and enabled on 127.0.0.1:8080."
else
    echo "ERROR: Nginx configuration validation failed! Checking syntax via nginx -t returned an error."
    echo "Removing invalid sandbox proxy file to prevent global Nginx failure..."
